...
var PUDIM = window.PUDIM = (function() {
		...
		modules = {
			universal_analytics: {
				...
				handler: function (url, post) {
					...
					function parseMicrosoft(item) {
						try {
							return JSON.parse(item.replace(/'/g, '"').slice(0, item.length-2)+'}')
						} catch(e) {
							return {};
						}
					}

					var obj = JSON.parse(post.text);
					if(!obj.data || !obj.data.item) return;

					var page_tags = parseMicrosoft(obj.data.item.pageTags)

					var conf = {};
					for(var i in page_tags) {
						conf['PageTags -> ' + i] = page_tags[i];
					}
					conf['PageName'] = obj.data.item.pageName;
					conf['uri'] = obj.data.item.uri;

					var keys = ['BrowserSize','PageSize','PageTitle','Scrl','VisitorId'];
					for(var i=0; i<keys.length; i++) {
						conf[keys[i]] = obj.data[keys[i]];
					}

					label = obj.data.item.pageName;
	...
	function init(details) {
		...
		if (window.commonRules.universal_analytics(url)) {
			if(details.request.postData) {
				modules.universal_analytics.handler(url, details.request.postData);
